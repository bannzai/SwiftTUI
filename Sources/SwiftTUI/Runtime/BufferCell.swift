...
public func bufferDrawBorder(
  ...
  for c in 1..<(width - 1) {
    bufferWriteCell(
      row: row, col: col + c, text: chars.horizontal, foregroundColor: color, into: &buffer)
  }
  bufferWriteCell(
    row: row, col: col + width - 1, text: chars.topRight, foregroundColor: color, into: &buffer)

  // 側面
  for r in 1..<(height - 1) {
    bufferWriteCell(
      row: row + r, col: col, text: chars.vertical, foregroundColor: color, into: &buffer)
    bufferWriteCell(
      row: row + r, col: col + width - 1, text: chars.vertical, foregroundColor: color,
      into: &buffer)
  }

  // 下辺
  bufferWriteCell(
    row: row + height - 1, col: col, text: chars.bottomLeft, foregroundColor: color, into: &buffer)
  for c in 1..<(width - 1) {
    bufferWriteCell(
      row: row + height - 1, col: col + c, text: chars.horizontal, foregroundColor: color,
      into: &buffer)
  }
  bufferWriteCell(
    row: row + height - 1, col: col + width - 1, text: chars.bottomRight, foregroundColor: color,
    into: &buffer)
}
