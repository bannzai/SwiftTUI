...
extension ViewBuilder._ConditionalContent: ViewGraphSetAcceptable {
    private func contentViewGraph(visitor: ViewGraphSetVisitor) -> ViewGraph {
        switch storage {
        case .truthy(let view):
            return visitor.visit(view)
        case .falsy(let view):
            return visitor.visit(view)
        }
    }
