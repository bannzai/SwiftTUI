...
public struct Picker<SelectionValue: Hashable>: View {
    ...
    public init(_ label: String, selection: Binding<SelectionValue>, options: [(value: SelectionValue, label: String)]) {
        self.label = label
...
// String配列用の便利初期化
public extension Picker where SelectionValue == String {
...
// Int配列用の便利初期化
public extension Picker where SelectionValue == Int {
    init(_ label: String, selection: Binding<Int>, options: [(Int, String)]) {
...
internal class PickerLayoutView<SelectionValue: Hashable>: LayoutView, FocusableView {
    ...
    init(selection: Binding<SelectionValue>, label: String, options: [(value: SelectionValue, label: String)], id: String) {
    ...
    func paint(origin: (x: Int, y: Int), into buffer: inout [String]) {
        ...
        if isExpanded {
            // 展開時の描画
            let labelLine = label + ": " + borderColor + "[" + currentLabel + " " + arrow + "]" + resetColor
            ...
            // 上枠線
            let topBorder = borderColor + "┌" + String(repeating: "─", count: maxWidth + 2) + "┐" + resetColor
            bufferWrite(row: origin.y + 1, col: origin.x + label.count + 3, text: topBorder, into: &buffer)
            ...
            for (index, option) in options.enumerated() {
                ...
                let line = borderColor + "│" + resetColor + highlightColor + " " + optionText + " " + resetColor + borderColor + "│" + resetColor
                bufferWrite(row: origin.y + 2 + index, col: origin.x + label.count + 3, text: line, into: &buffer)
            }
            
            // 下枠線
            let bottomBorder = borderColor + "└" + String(repeating: "─", count: maxWidth + 2) + "┘" + resetColor
            bufferWrite(row: origin.y + 2 + options.count, col: origin.x + label.count + 3, text: bottomBorder, into: &buffer)
        } else if isFocused {
            // フォーカス時（折り畳み）
            let contentWidth = label.count + 3 + currentLabel.count + 3
            
            // 上の枠線
            let topBorder = borderColor + "┌" + String(repeating: "─", count: contentWidth) + "┐" + resetColor
            bufferWrite(row: origin.y, col: origin.x, text: topBorder, into: &buffer)
            
            // コンテンツ行
            let content = borderColor + "│" + resetColor + label + ": [" + currentLabel + " " + arrow + "]" + borderColor + "│" + resetColor
            bufferWrite(row: origin.y + 1, col: origin.x, text: content, into: &buffer)
            
            // 下の枠線
            let bottomBorder = borderColor + "└" + String(repeating: "─", count: contentWidth) + "┘" + resetColor
