...
internal class AlertLayoutView: LayoutView, FocusableView {
    ...
    func paint(origin: (x: Int, y: Int), into buffer: inout [String]) {
        ...
        let titleLine = borderColor + "║" + resetColor + String(repeating: " ", count: titlePadding + 1) + 
                       boldStart + title + boldEnd + 
                       String(repeating: " ", count: maxWidth - title.count - titlePadding + 1) + 
                       borderColor + "║" + resetColor
        bufferWrite(row: currentRow, col: origin.x, text: titleLine, into: &buffer)
        currentRow += 1
        
        // 区切り線
        let separator = borderColor + "╟" + String(repeating: "─", count: maxWidth + 2) + "╢" + resetColor
        ...
        if let message = message {
            ...
            let messageLine = borderColor + "║" + resetColor + String(repeating: " ", count: messagePadding + 1) + 
                             message + 
                             String(repeating: " ", count: maxWidth - message.count - messagePadding + 1) + 
                             borderColor + "║" + resetColor
            bufferWrite(row: currentRow, col: origin.x, text: messageLine, into: &buffer)
            currentRow += 1
            
            // メッセージ後の空行
            let emptyLine = borderColor + "║" + resetColor + String(repeating: " ", count: maxWidth + 2) + borderColor + "║" + resetColor
        ...
        let buttonLine = borderColor + "║" + resetColor + String(repeating: " ", count: buttonPadding + 1) + 
                        buttonBg + buttonText + resetColor + 
                        String(repeating: " ", count: maxWidth - buttonText.count - buttonPadding + 1) + 
                        borderColor + "║" + resetColor
