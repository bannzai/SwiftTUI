...
public struct Slider<V: BinaryFloatingPoint>: View where V.Stride: BinaryFloatingPoint {
    ...
    public init(value: Binding<V>, in bounds: ClosedRange<V> = 0...1, step: V.Stride? = nil, label: String? = nil) {
        self._value = value
        self.bounds = bounds
        self.step = step
        self.label = label
...
// Double用の便利初期化
public extension Slider where V == Double {
    init(value: Binding<Double>, in bounds: ClosedRange<Double> = 0...1, label: String? = nil) {
        self.init(value: value, in: bounds, step: nil, label: label)
    }
}

/// SliderのLayoutView実装
internal class SliderLayoutView<V: BinaryFloatingPoint>: LayoutView, FocusableView where V.Stride: BinaryFloatingPoint {
    let value: Binding<V>
    let bounds: ClosedRange<V>
    let step: V.Stride?
    let label: String?
    ...
    func paint(origin: (x: Int, y: Int), into buffer: inout [String]) {
        ...
        // 値を0-1の範囲に正規化
        let normalizedValue = Double((value.wrappedValue - bounds.lowerBound) / (bounds.upperBound - bounds.lowerBound))
        ...
        // スライダーバーを作成
        let sliderBar = "[" + String(repeating: "█", count: filledCount) + String(repeating: "░", count: emptyCount) + "]"
