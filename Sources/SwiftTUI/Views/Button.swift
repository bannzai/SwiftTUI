...
internal class ButtonLayoutView<Content: View>: LayoutView, CellLayoutView, FocusableView {
  ...
  func paintCells(origin: (x: Int, y: Int), into buffer: inout CellBuffer) {
    ...
    if CellRenderLoop.DEBUG {
      ...
      print(
        "[Button] paintCells for button \(id) at (\(origin.x), \(origin.y)), size: \(width)x\(height)"
      )
      print("[Button]   Node address: \(node.rawPtr)")
      print("[Button]   Has children: \(YGNodeGetChildCount(node.rawPtr))")
      if width == 0 || height == 0 {
    ...
    guard YGNodeGetChildCount(node.rawPtr) > 0,
      let labelRaw = YGNodeGetChild(node.rawPtr, 0)
    else { return }
    ...
    // 上の枠線
    bufferWriteCell(
      row: origin.y, col: origin.x, text: "┌", foregroundColor: borderColor, into: &buffer)
    for i in 1..<(totalWidth - 1) {
      bufferWriteCell(
        row: origin.y, col: origin.x + i, text: "─", foregroundColor: borderColor, into: &buffer)
    }
    bufferWriteCell(
      row: origin.y, col: origin.x + totalWidth - 1, text: "┐", foregroundColor: borderColor,
      into: &buffer)
    ...
    for i in 0..<(totalHeight - 2) {
      // 左枠
      bufferWriteCell(
        row: origin.y + 1 + i, col: origin.x, text: "│", foregroundColor: borderColor, into: &buffer
      )
      ...
      if let bg = fillColor {
        for j in 1..<(totalWidth - 1) {
          bufferWriteCell(
            row: origin.y + 1 + i, col: origin.x + j, text: " ", backgroundColor: bg, into: &buffer)
        }
      }

      // 右枠
      bufferWriteCell(
        row: origin.y + 1 + i, col: origin.x + totalWidth - 1, text: "│",
        foregroundColor: borderColor, into: &buffer)
    }
    ...
    // 下の枠線
    bufferWriteCell(
      row: origin.y + totalHeight - 1, col: origin.x, text: "└", foregroundColor: borderColor,
      into: &buffer)
    for i in 1..<(totalWidth - 1) {
      bufferWriteCell(
        row: origin.y + totalHeight - 1, col: origin.x + i, text: "─", foregroundColor: borderColor,
        into: &buffer)
    }
    bufferWriteCell(
      row: origin.y + totalHeight - 1, col: origin.x + totalWidth - 1, text: "┘",
      foregroundColor: borderColor, into: &buffer)
  }
