...
#### 標準Observableを使用しなかった理由

Swift標準ライブラリのObservation framework (iOS 17+)を調査した結果、以下の理由でSwiftTUI独自のObservableを実装：

1. **マーカープロトコル問題**: SwiftのObservableは「マーカープロトコル」で、実際の実装は@Observableマクロがコンパイル時に生成
2. **手動実装不可**: マクロなしでの手動実装方法が公開されていない
3. **SwiftUIとの密結合**: SwiftUI専用に設計されており、TUI環境での動作は未検証
4. **マクロ非使用方針**: SwiftTUIはマクロを使用しない方針

#### SwiftTUI Observableアーキテクチャ

```swift
// 階層構造
SwiftTUIObservable (プロトコル)
